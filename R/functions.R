...
# to normalize the input data for MLP,LSTM, and GRU according to the variant
# @ x:        a vector, data frame or matrix to be transformed column wise
# @ Variant:  string giving the variant of normalization. Either "stdnorm" which 
#             standardizes and normalizes the data or "norm1" which scales them to be between 0 and 1


normalize <- function(x, variant = "stdnorm") {
  if(variant == "norm1"){
...
################################
# transform a scaled vector back to the original scale

# @ variant:  Variant used to normalize the data: "stdnorm" or "norm1"
# @ unscale:  vector giving either c(sd,mean) for "stdnorm" or c(min,max) for "norm1"
...
# @ epochs_opt:       number of optimization epochs during Bayesian optimization
# @ initPoints:       number of initial points calculated for Bayesian optimization
# @ duplicate:        vector giving info about duplication of min and max values for training: c(min proportion, max proportion, times) 
#                     ex.: c(0.1,0.9,1) adds the highest/lowest 10% of the values 1 time
...
# This Function returns a pdf with two qqplots


# @ measured:   vector with measured time series
# @ modeled:    vector with modeled time series
# @ path:       String with the file path to save the plot
# @ mod_type:   String with the name of the model type used for the plot title
# @ catchment:  String with the name of the catchment used for the plot title
...
#############################################
# Plot the measured/modeled time series and the model error with the measured/modeled mean
#############################################

# This Function returns a pdf with the plot of modeled/measured time series

# @ measured:   vector with measured time series
# @ modeled:    vector with modeled time series
# @ path:       String with the file path to save the plot
# @ mod_type:   String with the name of the model type used for the plot title
# @ xlab:       String giving the x-lab used in the plot
# @ years:      vector giving the years used in the x-axis
# @ catchment:  String with the name of the catchment used for the plot title

ts_plot <- function(measured, modeled, catchment, mod_type, xlab, years, path){
  pdf(file = path, width = 21, height = 7)
  ...
  plot(measured, type = "l", col="blue", ylim = c(miny,maxy), 
       main = paste(catchment, mod_type), 
       ylab = expression("Discharge [" ~ m^{3}/s ~ "]"),
       xlab = xlab,
       xaxt = "n")
  axis(side = 1, 
       at=seq(from = 182, to = length(measured)-50, by = 365),
       labels = years)
  lines(modeled, col="chartreuse4")
  lines(modeled-measured, col="red")
  abline(h=0)
  abline(h = mean(measured), col = "blue")
  abline(h = mean(modeled), col = "chartreuse4")
  abline(v=seq(from = 365, to = length(measured)-100, by = 365), lty = "dotted", lwd = 2)
  legend("topright", legend = c("measured" ,"modeled", "model error"), bty = "n", 
         lty = 1, col = c("blue", "chartreuse4", "red"))
  dev.off()
}
...
# @ model:        model if model type is xgb or lgbm to create the importance plots
# @ date:         vector giving the dates of the data points
...
analyze_model <- function(measured, 
  ...
  if(mod_type=="xgb"){
    ...
    # Plots the importance of predictors of the model (top 15)
    pdf(file = path2)
  ...
  if(mod_type=="lstm") {
    ...
    # shorten the measured data and dates to match the modeled
    skip <- length(modeled)-length(measured)
    measured <- tail(measured, n = skip)
    date <- tail(date, n = skip)
  ...
  if(mod_type=="svr"){
    ...
    # create paths for plots
    path1 <- paste("../Results/Plots/", catchment, "_SVRegression_ts.pdf", sep = "")
  ...
  # return a data frame with the measured and modeled data
  output <- data.frame(date, measured, modeled)
  return(output)
}
